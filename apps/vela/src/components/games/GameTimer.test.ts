...
const getTimeLeft = (wrapper: VueWrapper) => {
  const vm = wrapper.vm as unknown as { timeLeft?: number };
  if (typeof vm.timeLeft !== 'number') {
    throw new Error('timeLeft is not available on component instance');
  }
...
describe('GameTimer', () => {
  ...
  it('should decrement time every second', async () => {
    ...
    expect(wrapper.text()).toContain('Time: 60');
    expect(getTimeLeft(wrapper)).toBe(60);
    ...
    expect(getTimeLeft(wrapper)).toBeLessThan(60);
    expect(getTimeLeft(wrapper)).toBeGreaterThanOrEqual(58); // Should be around 59 or 58
    ...
    expect(getTimeLeft(wrapper)).toBeLessThan(59);
    expect(getTimeLeft(wrapper)).toBeGreaterThanOrEqual(57);
  });
  ...
  it('should countdown from 60 to 0', async () => {
    ...
    expect(getTimeLeft(wrapper)).toBeLessThanOrEqual(0);
