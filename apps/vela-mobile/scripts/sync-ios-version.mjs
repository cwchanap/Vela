...
//
// The resolution functions are exported for unit testing; the pbxproj write
// only runs when this file is the main entry point (not when imported).
...
const pbxprojPath = resolve(root, 'src-capacitor/ios/App/App.xcodeproj/project.pbxproj');
...
if (isMain) {
  ...
  // (`development` for `quasar dev`, `production` for `quasar build`). Default
  // to `production` since iOS MARKETING_VERSION matters most for release builds.
...
/**
 * Resolves `VITE_APP_VERSION` with the same precedence Vite's `loadEnv` uses:
 * `processEnv.VITE_APP_VERSION` first (shell export or Bun-loaded `.env`),
 * then the .env files Vite loads for the active mode (later files override
 * earlier), then package.json "version" as the final fallback. Returns the
 * resolved version plus the individual sources for diagnostics. Pure — no side
 * effects, no pbxproj access.
 */
export function resolveVersion({ root, mode, processEnv = process.env }) {
  const pkgVersion = JSON.parse(readFileSync(resolve(root, 'package.json'), 'utf8')).version;
  const processEnvVersion = processEnv.VITE_APP_VERSION;
  const fileEnvVersion = resolveFileEnvVersion(root, mode, processEnv);
  const version = processEnvVersion || fileEnvVersion || pkgVersion;
  return { version, processEnvVersion, fileEnvVersion, pkgVersion };
}
